<?php
/**
 * Guides Child functions and definitions
 *
 ...
 */
function guides_child_theme_setup() {
...
function guides_child_enqueue_styles() {
	...
	wp_enqueue_style( 'guides-child-style',
		get_stylesheet_directory_uri() . '/style.css',
		array('guides-main-style') //make sure the the child's style.css comes after the parents so you can overwrite rules
	);
}
add_action( 'wp_enqueue_scripts', 'guides_child_enqueue_styles' );
...
function guides_child_overwrite_files() {
	...
	// See the parent's function.php -> the guides_scripts_styles() function
	// for details like resources names

		wp_dequeue_script( 'guides-main-scripts' );
	...
	// This is not required, but I assume you are not modifying that much :)
		...
		wp_enqueue_script( 'guides-child-scripts',
...
// Load the files from the function mentioned above:
	...
	add_action( 'wp_enqueue_scripts', 'guides_child_overwrite_files', 11 );
